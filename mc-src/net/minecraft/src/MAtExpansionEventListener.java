package net.minecraft.src;

/*
* ----------------------------------------------------------------------------
* "THE COLA-WARE LICENSE" (Revision 0):
* Hurricaaane wrote this file. As long as you retain this notice you
* can do whatever you want with this stuff. If we meet some day, and you think
* this stuff is worth it, you can buy me a cola in return
* Georges "Hurricaaane" Yam
* ----------------------------------------------------------------------------
*/

public interface MAtExpansionEventListener
{
	void addedExpansion(MAtExpansion expansion);
	
	void removedExpansion(MAtExpansion expansion);
	
	void addedLimbo(String limbo);
	
	void successLimbo(String limbo);
	
	void failureLimbo(String limbo);
	
	void clearedExpansions();
	
}
