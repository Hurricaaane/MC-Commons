...
public class Ha3SoundCommunicator
{
	...
	Ha3SoundCommunicator(Ha3Mod modIn, String prefix)
	{
		this.mod = modIn;
		this.prefix = prefix;
		maxIDs = 256;
	...
	{
		if (sndManager == null)
			sndManager = mod.manager().getMinecraft().sndManager;
	...
	public boolean loadSoundSystem() throws Ha3ModPrivateAccessException
	{
		if (sndSystem == null)
			sndSystem = (SoundSystem) mod.manager().getPrivateValue(
					net.minecraft.src.SoundManager.class,
					mod.manager().getMinecraft().sndManager, 0);
			...
			float pitch)
	{
		...
		try
		{
			...
			// XXX Get rid of private value getting on runtime
			SoundPoolEntry soundpoolentry = ((SoundPool) mod.manager()
			...
			float pitch, int attnm, float rollf)
	{
		...
		try
		{
			...
			float soundVolume = mod.manager().getMinecraft().gameSettings.soundVolume;
